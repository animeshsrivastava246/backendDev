...
const getChannelStats = asyncHandler(async (req, res) => {
    ...
    const totalSubscribers = await Subscription.aggregate([
        {
            $match: {
                channel: new mongoose.Types.ObjectId.cacheHexString(userId),
                // channel: new mongoose.Types.ObjectId(userId) // use this if above does not work
            },
        },
        {
            $group: {
    ...
    const videos = await Video.aggregate([
        {
            $match: {
                owner: new mongoose.Types.ObjectId.cacheHexString(userId),
                // owner: new mongoose.Types.ObjectId(userId) // use this if above
            },
        },
        {
            $lookup: {
